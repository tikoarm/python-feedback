...
def get_profile_keyboard(is_admin: int):
    ...
    if is_admin == 1:
        keyboard.add(InlineKeyboardButton(text="💎 Admin 💎", callback_data="show_admin_panel"))
    return keyboard
...
def global_admins_buttons():
    keyboard = InlineKeyboardMarkup(row_width=1)
    keyboard.add(
        InlineKeyboardButton(text="📝 Global Information 📝", callback_data="admin_info"),
        InlineKeyboardButton(text="🛰️ Show API Keys 🛰️", callback_data="admin_show_all_api"),
        InlineKeyboardButton(text="🛰️ Create API Key 🛰️", callback_data="admin_create_api_key"),
